...
$dataLimite = $_POST['user_dataLimite'];
...
$search_element = mysqli_query($conexao, "SELECT * FROM requisicoes_de_doacoes WHERE email = '$email' AND status = 'AGUARDANDO'");
...
if (mysqli_num_rows($search_element) == 0) {
    ...
    if (!$insert) {
        ...
        echo "<script>alert('Alimento requisitação com sucesso. Aguarde respostas.')</script>";
    }
} else {
...
header("refresh: 0.5; url = desejo_receber.php");
